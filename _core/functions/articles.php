...
**/
	...
	$url=strtolower($url);
	$qurrey=mysql_query("select count(*) from data where LOWER(LINK)='$url' and STATUS=1");
...
**/
	...
	$article_url=mysql_real_escape_string($article_url);
	$qurrey=mysql_query("select count(*) from data where LINK = BINARY('$article_url') and STATUS=1");
	return (mysql_result($qurrey,0) == 1) ? true : false;
}
